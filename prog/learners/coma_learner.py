...
class COMALearner:
    ...
    Attributes:
        ...
        critic_training_steps: number of training steps for critic.
        log_states_t: state logging at timestep.
        critic: critic Q network.
        target_critic: target critic Q network.
        agent_params: parameters of agent network.
        critic_params: parameters of critic network.
        params: agent + critic params.
        agent_optimiser: policy optimizer.
        critic_optimiser: critic optimizer.
    """
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        q_vals, critic_train_stats = self._train_critic(batch,
                                                        rewards,
                                                        terminated,
                                                        actions,
                                                        avail_actions,
        ...
        if (self.critic_training_steps -
            self.last_target_update_step) / self.args.target_update_interval >= 1.0:
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            for key in ["critic_loss", "critic_grad_norm",
                ...
                self.logger.log_stat(key, sum(critic_train_stats[key])/ts_logged,
                                     t_env)
            ...
            self.logger.log_stat("pi_max", (pi.max(dim=1)[0] * mask).sum().item()
                                 / mask.sum().item(), t_env)
            self.log_stats_t = t_env

    def _train_critic(self, batch, rewards, terminated,
                      actions, avail_actions, mask, bs, max_t):
        """Train critic network.
        
        Args:
            batch: data sample batch.
            rewards: agent rewards.
            terminated: whether timestep terminated or not.
            actions: agent actions.
            avail_actions: available actions.
            mask: mask data samples.
            bs: batch size.
            max_t: max episode length.
        """
        ...
        for t in reversed(range(rewards.size(1))):
            ...
            q_taken = th.gather(q_t, dim=3,
                                index=actions[:, t:t+1]).squeeze(3).squeeze(1)
            targets_t = targets[:, t]
            ...
            running_log["td_error_abs"].append((masked_td_error.abs().sum().item()
                                                / mask_elems))
            running_log["q_taken_mean"].append((q_taken * mask_t).sum().item()
                                               / mask_elems)
            running_log["target_mean"].append((targets_t * mask_t).sum().item()
                                              / mask_elems)
